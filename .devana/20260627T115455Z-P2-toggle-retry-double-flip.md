...
- 2026-06-27: reopened after validation. The dashboard descriptor path is now idempotent, but the public `toggle` route itself still implements a bare POST as a relative `!current.enabled` flip. README also documents that the relative `toggle` route remains available for clients. A retry of the same bare `POST /toggle` still reads the post-first-write state and flips back, so the original route-level retry counterexample is not blocked.
- 2026-06-27: fixed. `toggleRoute` no longer derives a relative flip from an empty body. Content-only `message`/`messages` patches still preserve `current.enabled`, but state changes must include an explicit `enabled` boolean; an empty bare toggle now returns a bad request instead of reading, inverting, and writing stale state. Duplicate deliveries with the same explicit target remain idempotent. README and regression coverage were updated to describe and verify the explicit-target contract.
